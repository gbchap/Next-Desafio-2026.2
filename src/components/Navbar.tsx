...
export default function Navbar(){
    ...
    const desktopLinkClass = (href: string) =>
        `${linkStyle} cursor-pointer pb-1 ${pagAtual(href) ? "border-b-2 border-deepgreen" : ""}`;
        ...
        <nav className="bg-pinkish px-6 py-4 relative">
            <div className="flex items-center justify-between">
                ...
                <ul className="hidden md:flex gap-12">
                    <li>
                        <Link href="/home" className={desktopLinkClass("/home")}>HOME</Link>
                    </li>
                    <li>
                        <Link href="/contato" className={desktopLinkClass("/contato")}>CONTATO</Link>
                    </li>
                    <li>
                        <Link href="/produtos" className={desktopLinkClass("/produtos")}>PRODUTOS</Link>
                    </li>
                    <li>
                        <Link href="/gerenciamento" className={desktopLinkClass("/gerenciamento")}>GERENCIAMENTO</Link>
                    </li>
                    <li>
                        <Link href="/login" className={desktopLinkClass("/login")}>LOGIN</Link>
                    </li>
                    <li>
                        <Link href="/carrinho" className={desktopLinkClass("/carrinho")}>CARRINHO</Link>
                    </li>
                </ul>

                <button
                    onClick={() => setIsOpen(!isOpen)}
                    className="md:hidden flex flex-col gap-1.5 relative z-50 cursor-pointer"
                >     
                    {isOpen ?(
